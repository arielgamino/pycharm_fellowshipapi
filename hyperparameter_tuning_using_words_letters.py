"""
This code takes from the Detect Language - Attempt 3 notebook and
it creates a way to try different hyperparameters for modeling tuning.

Three hyperparameters:
number_of_documents - how many documents from corpora to use for training
number_of_words     - how many of the most common words to use on the feature set
number_of_letters   - how many of the most common letters to use on the feature set

It consist of 5 steps:

Step 1 - Read corpora from preprocessed pickles files
Step 2 - Extract most_common words, and most_common_letters
Step 3 - Divide data set into training and test set
Step 4 - Build model
Step 5 - Test a sample of eruparl.test against the model
...
for number_of_documents in [1000,2000]:
    # -------------Step 1-------------
    ...
    for words_letters in [(100, 10), (400,100)]:
        ...
        results_outfile = "stats/europarl_test_classified_attempt_" + str(number_of_documents) + "_" + str(number_of_words) + "_" + str(number_of_letters) + ".csv"
        everyother = 20
        start = time.time()
...
writer = pd.ExcelWriter('hypertuning_results_words_and_letters.xlsx')
